...
def CalculateForces2(pos, epsilon, sigma, num_particles, L):
    ...
    delxyz = (pos[np.newaxis,:,:] - pos[:,np.newaxis,:]) - np.rint((pos[np.newaxis,:,:] - pos[:,np.newaxis,:])/L)*L
...
for i in range(0, simulation_steps):
    ...
    if(i % vid_sample_frames == 0):
        ...
        if two_particles ==1 and i >= 30000:
            dens_bins1, dens_bins2 = DensityCalc(pos,dens_bins1,dens_bins2,box_size,dz,num_particles)
...
if two_particles == 1:
    ...
    dens_bins1 = dens_bins1/((simulation_steps-30000)/vid_sample_frames)
    dens_bins2 = dens_bins2/((simulation_steps-30000)/vid_sample_frames)
